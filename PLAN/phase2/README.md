...
> SkillSystem 已迁移到 Phase 3，计划见 `PLAN/phase3/README.md` 和 `PLAN/phase3/plan1.md`。
...
> 把项目从 Web 自动化 runtime，升级为可恢复、可确认、可解释的 Agent Kernel。Web 只是第一个垂直执行域，不能成为底层架构的全部。
...
SkillSystem 不再作为 Phase 2 的实施内容。

Phase 2 到 Plan 8 聚焦底层 Agent Kernel：run/session、tool execution、permission/approval、context compaction、workflow evidence、completion gate、resume completion。

SkillSystem 属于“经验层 / 可复用能力层”，已迁移到 Phase 3：
...
当这些能力成立后，Phase 3 再继续做 SkillSystem / Memory / Extension 这类经验层和扩展层能力。
